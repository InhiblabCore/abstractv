...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props) {
      ...
      const { usedFilters, editFilterName, removeFilter, saveFilter } = inject(
        'filterManager',
      ) as any

      const { enabledFilters, onUsedChange, updateIndicator } = inject('filterConfig', {}) as any

      const { dataStatus } = inject('sourceDrawer', {}) as any
      ...
      const refreshData = async (c: string) => {
        props.dataFilter.origin = c
        ...
        return saveFilter(props.dataFilter)
      }
