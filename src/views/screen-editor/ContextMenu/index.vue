<template>
  <div v-if="contextMenu.show" class="context-menu-wrap" :style="contextMenuStyle">
    ...
    <div class="context-menu-item" @click="lockComponent">
    ...
    <div class="context-menu-item" @click="hideComponent">
    ...
    <div class="context-menu-item" @click="deleteComponent">
      <n-icon class="menu-icon">
        <IconDelete />
      </n-icon>
      删除
...
<script lang="ts" setup>
  ...
  const lockComponent = () => {
    editorComStore.setComponentLocked(selectedCom.value.id, !isLocked.value)
  }

  const hideComponent = () => {
    editorComStore.setComponentHided(selectedCom.value.id, !isHided.value)
  }
  ...
  const copyComponent = () => {
    if (selectedCom.value) {
      editorComStore.copyComponent(selectedCom.value.id)
    }
  }

  const deleteComponent = () => {
    const id = selectedCom.value.id
    if (id) {
      const d = nDialog.create({
        ...
        onPositiveClick: async () => {
          d.loading = true
          try {
            await editorComStore.deleteComponent(id, selectedCom.value.type)
          } catch (error: any) {
